...
{{
  ...
  "venue": "会议或期刊名称（如果标题或摘要中提到，如 'ICLR', 'NeurIPS', 'CVPR', 'Operations Research', 'Transportation Research Part C' 等，如果没有提到则为空字符串）",
  "venue_year": 年份（整数，如 2026，如果没有提到则为 null）,
  "institutions": ["知名机构列表（从标题、摘要或作者信息中提取，如 'MIT', 'Stanford', 'Google', 'Tsinghua University', 'ByteDance' 等，如果没有则为空列表）"]
}}

**关于 venue 和 institutions 的提取说明**：

1. **venue（会议/期刊）提取**：
   - 从标题中查找会议/期刊信息，常见格式如："Accepted to ICLR 2026", "Published in NeurIPS 2025", "In CVPR 2024"
   - 如果标题中没有，检查摘要开头是否提到
   - 只提取顶级会议/期刊的标准缩写，如：NeurIPS, ICML, ICLR, CVPR, AAAI, IJCAI, ACL, KDD, Operations Research, Transportation Research Part C 等
   - 如果无法确定，返回空字符串 ""

2. **venue_year（年份）提取**：
   - 从标题或摘要中提取会议/期刊的年份
   - 只提取明确的年份数字（如 2026, 2025, 2024）
   - 如果无法确定，返回 null

3. **institutions（机构）提取**：
   - 从标题、摘要或作者信息中识别知名机构
   - 包括：
     * 顶级大学：MIT, Stanford, Berkeley, CMU, Harvard, Oxford, Cambridge, Tsinghua, Peking, NUS, ETH 等
     * 知名公司：Google, OpenAI, Meta, Microsoft, ByteDance, Alibaba, Tencent, Baidu 等
   - 使用标准全称或常用缩写，如：
     * "Massachusetts Institute of Technology" 或 "MIT"
     * "Tsinghua University" 或 "THU"
     * "Google Brain" 或 "Google"
   - 如果没有识别到知名机构，返回空列表 []
   - 最多返回 3-5 个最重要的机构
...
摘要："We propose a novel approach... Authors from MIT and Google Brain..."

输出：
{{
  "score": 8.5,
  "reason": "...",
  "title_zh": "深度强化学习用于车辆路径问题",
  "abstract_zh": "...",
  "tags": ["强化学习", "组合优化", "车辆路径问题"],
  "venue": "ICLR",
  "venue_year": 2026,
  "institutions": ["MIT", "Google Brain"]
}}
