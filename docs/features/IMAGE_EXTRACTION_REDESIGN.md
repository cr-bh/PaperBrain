...
7. ✅ **修复 Table 水平线上边界限制 + 降级文本内容定位**（见十六节）
8. ✅ **修复 Figure 被附录 Table 挤出 + Table 脚注扫描纳入相邻图内容**（见十七节）
9. ✅ **修复 Figure 跨栏补充检测误判导致截图包含相邻栏内容**（见十八节）
9. 🔲 **TODO：Algorithm 专用裁剪路径**（见十四节）
10. 🔲 使用至少 3 篇不同类型论文（纯光栅图、纯矢量图、混合）验证提取结果
...
- [x] Table 水平线上边界限制，不越过相邻 Table 的水平线
- [x] Table Strategy 1b 降级时用文本内容底部，不用页面底部
- [x] 附录大量 Table 不再挤掉正文核心 Figure（FIGURE_QUOTA 配额保护）
- [x] Table 脚注扫描不再纳入相邻栏图表的坐标轴标签（栏内过滤 + 间隔阈值收紧）
- [x] Figure 跨栏补充检测不再因坐标轴溢出误判为跨栏（改用 crop 在原栏内比例 ≥ 80% 判断）
...
## 十六、已修复：Table 截取两处问题（v1.5）

**commit**: `7c9c9e0`
**涉及文件**：`services/pdf_parser.py`

### 问题 1：Table 6 截取了下方正文

**现象**：Table 6 的 crop 底部延伸到页面底部（y=772），将表格下方大量正文一并截入。

**根因**：Table 6 水平线宽度 164pt（约 29% 栏宽），低于 Strategy 1b 的 50% 阈值，被过滤为无效线。Strategy 1b 找不到水平线时，直接降级使用 `next_caption_y`（下一个 caption 的 y0，此处为页面底部 772pt）作为裁剪底部，导致包含大量正文。

**修复**：Strategy 1b 降级路径新增文本内容扫描：在 `caption.y1` 到 `scan_limit`（`search_bottom` 或 `caption.y1+300`）范围内逐块扫描文本，遇到第一个正文段落（`avg_len > 50`）时停止，用实际表格文本底部（y=189.2）代替页面底部。

```python
# Strategy 1b 降级：扫描表格文本块，遇正文段落停止
if page is not None:
    ...
    for b in sorted(blocks, key=lambda x: x[1]):
        if by0 <= cap_rect.y1 or by0 >= scan_limit:
            continue
        avg_len = len(text) / max(len(lines), 1)
        if avg_len > 50:  # 正文段落，表格内容已结束
            break
        text_bottom = by1
...
**验证**：Table 6 crop bottom=199.2，正文从 y=212.8 开始，不包含正文 ✅

---

### 问题 2：Table 8 截取了 Table 9 的完整内容

**现象**：Table 8 的 crop 底部延伸到 y=267.8，将 Table 9 的全部内容（y=182.7~268）一并截入。

**根因**：Strategy 1b 扫描"caption 下方所有水平线"时没有上边界限制，Table 9 的水平线（y=194~268）也满足「在 caption 下方」的条件，被纳入 Table 8 的底部计算，导致 `table_bottom=267.8`（Table 9 最后一条线）。

**修复**：`_find_content_bbox_via_drawings` 新增 `search_bottom` 参数；`_determine_crop_rect` 的 Table 分支计算下一个同栏 caption 的 y0 并传入，Strategy 1b 扫描时跳过 `r.y0 >= search_bottom` 的水平线。

```python
# _determine_crop_rect：计算下一个同栏 caption 的 y0
same_col_hits = [h for h in caption_hits if h is not hit and self._in_same_column(...)]
next_ys = [h["rect"].y0 for h in same_col_hits if h["rect"].y0 > cap_rect.y1 + 10]
...
bbox = self._find_content_bbox_via_drawings(
    ...
    search_bottom=next_caption_y  # 新增：限制扫描上边界
)

# _find_content_bbox_via_drawings Strategy 1b：
if search_bottom > 0 and r.y0 >= search_bottom:
    continue  # 不越过下一个 caption
table_hlines.append(r.y0)
```

**验证**：Table 8 crop bottom=170.0，不包含 Table 9（y=182.7）✅；ATTENTION RESIDUALS Table 2 crop bottom=224.8，不含 APPENDIX ✅；AL_iLQR Algorithm 6 尺寸不变 ✅
...
| 问题 | 根因 | 修复 |
|------|------|------|
| Table 6 包含下方正文 | 水平线 164pt < 50% 阈值，降级用页面底部 | 降级改为扫描文本块，遇正文段落停止 |
| Table 8 包含 Table 9 | Strategy 1b 无上边界，纳入 Table 9 的线 | 新增 `search_bottom` 参数，限制到下一个 caption y0 |

---

## 十七、已修复：Figure 被附录 Table 挤出 + Table 脚注扫描纳入相邻图内容（v1.6）

**测试 PDF**：`iclr26-HEURIGYM.pdf`（55 页，含大量附录 Table，双栏，正文有 Figure 1 框架图）
**commit**: 待提交
**涉及文件**：`services/pdf_parser.py`

### 问题 1：Figure 1 没有被截取

**现象**：论文正文第 4 页的 `Figure 1: Overview of the HeuriGym agentic framework` 未出现在提取结果中。

**根因**：全局选择逻辑 `all_candidates.sort(key=lambda c: c["priority"])[:MAX_TOTAL_IMAGES]` 只按图片类型优先级排序，不考虑页码。该论文附录（page 29~55）有 20+ 个 Table（priority=1），全部排在 Figure 1（priority=2，architecture 类型）之前，将 Figure 1 挤出 Top 12。

**修复**：两阶段选择策略：
1. 先从非 table 类型（figure/architecture/performance/algorithm）中按 `(priority, page)` 取 `FIGURE_QUOTA=4` 张，保证正文核心图不被挤掉
2. 剩余 `MAX_TOTAL_IMAGES - FIGURE_QUOTA = 8` 个名额用 table 填充（同样按 `(priority, page)` 排序，正文 table 优先于附录 table）
...
selected = non_tables[:quota_non_table] + tables[:self.MAX_TOTAL_IMAGES - quota_non_table]
```

**验证**：Figure 1（1144×353px）成功进入 Top 12 ✅

---

### 问题 2：Table 5 截取了下方 Figure 4 的图表内容

**现象**：Table 5（右栏，y=81.9~186.1）的 crop bottom 延伸到 y=277，将 Figure 4 的坐标轴标签（"0.8", "0.6" 等）和图例（"Gemini-2.5-Flash", "Quality (Q)" 等）一并截入。

**根因**：Strategy 1b 脚注扫描的两个缺陷：
1. **无栏内过滤**：脚注扫描未检查文本块 x 范围是否在当前栏内，导致左栏（x=261~299）的 Figure 3 饼图标签（"API Error..."）被纳入
2. **间隔阈值过大（15pt）**：Table 5 底线 y=186.1，Figure 4 第一个标签 y=200.5，间隔 14.4pt < 15pt，未触发停止；真正的脚注（† ‡ 注释）紧跟表格底线，间隔通常 < 5pt

**修复**：
1. 脚注扫描添加栏内过滤：`if bx0 < col_x0 - 10 or bx1 > col_x1 + 10: continue`
2. 连续性间隔阈值从 15pt 收紧为 8pt

**验证**：Table 5 crop bottom=196.1，不包含 Figure 4 内容（y≥204）✅；ATTENTION RESIDUALS 所有图片尺寸不变 ✅
...
| 问题 | 根因 | 修复 |
|------|------|------|
| Figure 1 被附录 Table 挤出 Top 12 | 全局排序只按类型优先级，不考虑页码 | 两阶段选择：非 table 类型保留 FIGURE_QUOTA=4 配额 |
| Table 5 脚注扫描纳入相邻图内容 | 无栏内过滤 + 间隔阈值 15pt 过大 | 添加栏内 x 过滤 + 间隔阈值收紧为 8pt |

---

## 十八、已修复：Figure 跨栏补充检测误判导致截图包含相邻栏内容（v1.6）

**测试 PDF**：`iclr26-HEURIGYM.pdf` page 9（Figure 3/4 并排，双栏）
**涉及文件**：`services/pdf_parser.py`

### 问题描述

`Figure 4: Quality-Yield tradeoff.`（右栏）的截图宽度为 1144px（全页宽），包含了左栏 Figure 3 的全部内容。

### 根因分析

**触发链**：

1. Figure 4 caption 在右栏（x=340.8~471.2），初始 `col=(334.9, 592)`
2. Strategy 2 的 drawings union bbox = `Rect(308, 199.1, 504, 282.6)`，`bbox.x0=308` 比右栏左边界 334.9 小 26.9pt
3. `_determine_crop_rect` 返回 `crop_rect.x0 = min(334.9, 308-3) = 305`
4. 跨栏补充检测：`crop_rect.x0=305 < col_x0=334.9 - 20=314.9` → 触发跨栏扩展，col 变为全页宽 (20, 592)
5. 全页宽下重新计算，coarse_top 仍为 Table 4/5 caption 底部（y≈97），图形内容从 y≈199 开始，crop 包含了 y=97~199 的空白区域和全页宽的图形内容（同时截入 Figure 3）

**根本原因**：跨栏补充检测用固定 20pt 容差判断，无法区分"坐标轴/坐标区域稍微溢出栏边界"（假跨栏）和"图形真正横跨两栏"（真跨栏）。

### 修复方案

将跨栏补充检测的判断标准从"crop_rect 超出当前栏 20pt"改为"crop_rect 在原始栏内的比例 < 80%"：

```python
crop_w = crop_rect.x1 - crop_rect.x0
overlap = max(0, min(col_x1, crop_rect.x1) - max(col_x0, crop_rect.x0))
in_col_ratio = overlap / crop_w  if crop_w > 0 else 1.0
...
**比例计算**：
- HEURIGYM Figure 4：overlap=169.1pt / crop_w=196pt = **86.3%** > 80% → 不触发跨栏 ✓
- ATTENTION RESIDUALS Figure 4：overlap=128pt / crop_w=182pt = **70.3%** < 80% → 触发跨栏 ✓

### 验证结果

| 图片 | 修复前 | 修复后 |
|------|--------|--------|
| HEURIGYM Figure 4 | 1144×221px（全页宽，含 Figure 3） | **575×220px**（右栏，仅 Figure 4） |
| HEURIGYM Figure 3 | 610×221px（正常） | **610×221px**（不变） |
| ATTENTION RESIDUALS Figure 4 | 792×464px | **792×464px**（不变） |
